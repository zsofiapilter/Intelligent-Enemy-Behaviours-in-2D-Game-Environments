...
public class EnemyMovementAStar : MonoBehaviour
{
    [Header("Grid")]
    [Tooltip("World-space cell size (meters per cell). 0.5–1.0 is common on tile games.")]
    public float cellSize = 1.0f;

    [Tooltip("How many cells outward from the enemy in +X/-X/+Y/-Y to allow pathing.")]
    public Vector2Int gridHalfExtents = new Vector2Int(40, 40);

    [Tooltip("Layers considered blocking.")]
    public LayerMask obstacleMask;

    [Tooltip("Box overlap fraction of a cell to test block. 0.8 means test a slightly smaller box than the cell.")]
    ...
    [Tooltip("Repath every N seconds regardless, to adapt to dynamic obstacles.")]
    public float hardRepathInterval = 0.75f;

    [Header("Stuck Handling")]
    [Tooltip("If we make less progress than this over the window, consider re-pathing.")]
    public float minProgressPerSecond = 0.02f;
    [Tooltip("Seconds over which we measure progress.")]
    public float progressWindowSeconds = 0.5f;

    [Header("A* Performance")]
    [Tooltip("Use 8-direction (diagonals). If false, only 4 neighbors.")]
    public bool allowDiagonal = true;
    [Tooltip("Maximum nodes to expand per search to prevent spikes. 0 = unlimited.")]
    ...
    private bool IsWalkable(Vector2Int cell)
    {
        ...
        Vector2 size = Vector2.one * (cellSize * occupancyFraction);
